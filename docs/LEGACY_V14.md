# 米国株司令室 ULTRA v14.0.0

Render の無料 Web Service で動く、個人向けの米国株ポートフォリオ／リスク管理 PWA です。取得品質、損失レンジ、参考株数、STOPリスク、複合ストレス、端末内復旧に加え、50テーマの相対強度、推定ローテーション、RRG型4象限を一画面で管理します。

## v14.0 テクニカル買いシグナル点灯システム

- 「買い」タブで、保有・監視・シャドー銘柄を対象に、日足／週足のRSI14、MACD 12/26/9、EMA20/50/200、出来高、RVOL、20日高値・安値、テーマ品質、決算・ニュース接続状況をまとめて確認できます。
- 8項目を `true / false / unavailable` で管理し、固定100点配点と減点を分離表示します。RSIだけでは買い候補にせず、履歴不足は `unavailable`、部分取得は `partial`、古い観測は `stale` として安全側へ制限します。
- RSI 80以上、5営業日で25%以上の急騰、決算前3営業日以内、リスクリワード1.8未満、日次レバレッジ商品は、点数が高くても買い候補へ昇格しません。レバレッジ商品は監視参考だけで、推奨金額・株数を返しません。
- エントリー、STOP、第1／第2利確、参考金額・株数は `SIMULATE` 専用です。自動発注・証券口座・注文APIには接続せず、画面操作も「監視追加」「SHADOW追加」だけです。
- APIは `GET/POST /api/buy-signals`、検証は `/api/buy-signals/backtest` です。Yahoo Chart v8から5年分の確定日足OHLCVを取得して短時間キャッシュし、週足は完成した週だけを集計します。Sparkが終値しか返さない実環境では自動的にChart v8へ切り替えます。
- バックテストA〜Eは、シグナル確定日の次営業日始値をエントリーに使い、手数料・スリッページ、70/30の時系列分割、5・10・20営業日後、勝率、平均損益、最大ドローダウン、Profit Factor、急落／通常局面を表示します。未来データは使用しません。
- 完成版Fには当時点のテーマ構成、ニュース公開時刻、決算予定の変更履歴、ファンダメンタル公開時刻が必要です。標準構成では完全なpoint-in-time履歴がないため、過去結果を捏造せず `unavailable` と理由を返します。
- Yahoo日足から真正なセッションVWAP／出来高プロファイルは復元できないため `unavailable` です。Finnhubキー未設定時の決算・ニュース・ファンダメンタルも0点やfalseで埋めず `unavailable` とします。

## v13.2.1 株価・為替・VIX・WTI 共通同期

- 画面右上の同期ボタン1回で、保有株・監視株・USD/JPY・VIX・WTI原油・SPY市場レジームをまとめて更新します。市場画面の専用更新ボタンからも同じ再検証を実行できます。
- 手動同期では15分の市場レジームキャッシュを越えて上流データを再検証します。短時間内の連続操作はキャッシュを再利用し（既定45秒）、重複リクエストは1つへ集約します。起動時・自動同期は従来どおりキャッシュを使い、Render無料枠の負荷を抑えます。
- 株価・為替とVIX・WTIの片側だけ成功した場合は「一部成功」と表示します。DATA CONNECTIONでVIX・WTIの値、共通確定日、取得状態、キャッシュ種別、失敗理由を確認できます。
- VIX・WTIは日次確定終値です。休場中や新しい終値が確定する前は、更新に成功しても値・基準日・dataRevisionが変わらない場合があります。

## v13.2 VIX・WTI MARKET RISK REGIME

- 市場タブの先頭で `^VIX`、WTI期近先物の連続系列 `CL=F`、`SPY` を同じ確定営業日にそろえ、最新確定終値と1・5・20営業日の変化、21営業日の相対推移を表示します。日中リアルタイムではなく、確定した日次終値による観測です。
- `危険 / 警戒 / 中立 / 楽観` は VIX の水準と加速、SPY の下落・反発、原油急変の組み合わせから判定します。VIXは約30日の予想変動率で株価方向の予測ではなく、WTIは需給・在庫・金融要因で動くため、原油の上昇・下落単独では判定しません。
- API は `/api/market-regime`、方式は `vix-wti-spy-rules-v3` です。`regime.state`、`riskScore`、`optimismScore`、`confidence`、`reasons` と、`instruments.vix/wti/spy` の `value`、`changes`、`trail` を返します。SPYは15分キャッシュのYahoo Chart調整後終値を優先し、取得できない場合はSpark未調整終値へ安全にフォールバックして`REFERENCE`表示になります。
- `confidence` はデータ品質で、将来確率ではありません。`riskScore` / `optimismScore` もルール寄与を相補表示したヒューリスティック指数であり、暴落確率・上昇確率・期待リターンではありません。`^VIX` は売買銘柄ではなく、`CL=F` は限月ロールの影響を含む期近先物連続系列です。このレジームは `tradeEligible: false` の観測補助で、Trade Gateや自動発注には接続しません。
- Yahoo の非公式エンドポイントを best-effort で使うため、raw終値、欠損、古い観測は `REFERENCE / WATCH ONLY` として明示します。VIXが極端に高い場合は他系列が欠けても危険側へ倒しますが、欠損状態で中立・楽観は確定しません。
- AIテーマ分析にはレジームの時点・revision・品質制約を別枠で渡し、VIX/WTI更新後の古い分析を現行分析として表示しません。

## v13.1 RRG型4象限

- 市場テーマOSに `RRG 4象限` を追加しました。X軸はテーマ構成4銘柄の63営業日等ウェイト対数リターンからSPYの同期間対数リターンを引いた値、Y軸は同じ方法の5営業日値です
- 右上をリーダー、右下を調整、左上を短期反発、左下を弱として表示します。0は右／上側へ分類します
- `-20D`、`-15D`、`-10D`、`-5D`、`-1D`、`最新`、前後1営業日、20日再生に対応します。軸の縮尺は全21フレームで固定し、点をタップすると従来のテーマ詳細・主要銘柄・AI分析を開きます
- 全テーマ、4象限、8分類、検索で絞り込めます。SVGと同内容の表、キーボード選択、320px幅、端末の「動きを減らす」設定にも対応します
- SPYを営業日カレンダーの正本とし、テーマ銘柄も完全に同じ現在日・63営業日前・5営業日前の終値だけを使います。欠損は0、前日値、近似日で補完しません
- Score側の1・5・20営業日も同じSPY営業日に厳密同期します。市場観測が古い、または必要日が欠ける場合は `partial / WATCH ONLY` とし、NEXT候補と動的edgeを停止します
- これは公式RRG指標ではなく、現行固定バスケットを過去へ当てた「RRG型・SPY対比の価格ローテーション参考図」です。実際の資金移動、注文フロー、因果、将来予測、投資助言ではありません
- Yahoo Sparkが調整後終値を返さない場合はraw終値へフォールバックし、画面・API・AI入力へ制約を明示します。raw終値のリターンは配当を含まず、株式分割などの企業行動で歪む可能性があるため、参考表示は維持しつつ売買候補・NEXT・動的edgeを無効化します
- AI分析には現在の象限、20営業日前と現在の座標、象限遷移、price mode、欠損、証拠IDを渡します。RRG位置を資金流入額として説明させません
- AI分析のrevisionは全30営業日履歴、RRG軌跡、品質、edge検証結果まで含めて作り直し、過去値だけが変わった場合も古い分析を現行扱いしません
...
この版の標準データ経路では、注文、約定方向、ファンドフロー、過去出来高を取得していません。画面上のFlow系表示は、調整後終値を優先し、取得できない場合はraw終値フォールバックを明示した価格系列の相対強度と、テーマ内の上昇銘柄比率から算出する**推定ローテーション指数**です。通貨額ではなく、実際の資金流入・流出を観測した値でもありません。APIは `actualFundFlow: false`、`volumeUsed: false` と推定方法を返します。Score 50は同日の対象テーマ群の中央値付近を意味し、「資金中立」を意味しません。Confidenceは予測的中確率ではなく証拠品質です。テーマは現在の固定バスケットを過去へ当てた等ウェイト参考値で、銘柄重複があるためテーマ同士は独立ではなく、構成入替前の状態も再現しません。
...
- MARKET THEME INTELLIGENCE APIを実Yahoo通信で確認。50テーマ、8分類、43構造edge、Score構成138銘柄、ETF等を含む全183シンボルとSPYを取得し、HTTP 200・約3.9秒（検証時）
- 実データでRRG型4象限を確認。50/50テーマの現在座標、21フレーム、46テーマの完全trailを算出し、残り4テーマは欠損日をN/Aのまま保持しました。APIはraw終値フォールバック、配当未調整、企業行動注意を返します
- APIと画面の両方で `actualFundFlow: false`、出来高・ニュース・時価総額未接続、stale/partial/raw時のNEXT・5条件ゲート・動的edge停止、内部計算field非公開を確認
- 古い市場観測、SPY基準日の途中欠損、raw/adjusted混在、株式分割相当の価格飛び、過去30日だけの変更、SPY単独障害をfixture化し、安全側で停止・正常キャッシュ保持・revision更新されることを確認
...
| `MARKET_REGIME_RATE_LIMIT_PER_MINUTE` | 任意 | VIX・WTI・SPYレジームAPIのIPごとの上限。既定12回/分 |
| `MARKET_REGIME_CACHE_TTL_MS` | 任意 | 同期済みレジーム観測のキャッシュ。既定900,000ms（15分） |
| `MARKET_REGIME_SPY_ADJUSTED_TTL_MS` | 任意 | SPY調整後終値（Yahoo Chart）の成功・失敗キャッシュ。既定900,000ms（15分） |
| `MARKET_REGIME_PARTIAL_CACHE_TTL_MS` | 任意 | 部分取得レジームの短期キャッシュ。既定120,000ms（2分） |
| `MARKET_REGIME_STALE_TTL_MS` | 任意 | 上流障害時に参考表示できる最長キャッシュ。既定86,400,000ms（24時間） |
| `MARKET_REGIME_DATA_DEADLINE_MS` | 任意 | VIX・WTI・SPY履歴の一括取得上限。既定12,000ms |
| `TECHNICAL_SIGNAL_RATE_LIMIT_PER_MINUTE` | 任意 | テクニカル買いシグナルAPIのIPごとの上限。既定6回/分 |
| `TECHNICAL_BACKTEST_RATE_LIMIT_PER_MINUTE` | 任意 | 2年バックテストAPIのIPごとの上限。既定2回/分 |
| `TECHNICAL_SIGNAL_ROUTE_TIMEOUT_MS` | 任意 | 買いシグナルHTTP応答の上限。既定35,000ms |
| `TECHNICAL_BACKTEST_ROUTE_TIMEOUT_MS` | 任意 | バックテストHTTP応答の上限。既定75,000ms |
| `TECHNICAL_SIGNAL_MAX_CONCURRENCY` | 任意 | 同時シグナル分析数。既定3、最大8 |
| `TECHNICAL_BACKTEST_MAX_CONCURRENCY` | 任意 | 同時バックテスト数。既定1、最大2 |
| `TECHNICAL_CACHE_TTL_MS` | 任意 | 5年OHLCV履歴の正常キャッシュ。既定900,000ms（15分） |
| `TECHNICAL_STALE_TTL_MS` | 任意 | 上流障害時に参考表示できるテクニカル履歴の最長キャッシュ。既定86,400,000ms |
| `TECHNICAL_DATA_DEADLINE_MS` | 任意 | テクニカル履歴・テーマ統合の一括取得上限。既定25,000ms |
| `TECHNICAL_UPSTREAM_TIMEOUT_MS` | 任意 | Yahooのテクニカル履歴1リクエスト上限。既定8,000ms |
| `TECHNICAL_YAHOO_BATCH_SIZE` | 任意 | Yahoo履歴の1バッチ銘柄数。既定20、最大20 |
| `TECHNICAL_CHART_CONCURRENCY` | 任意 | SparkがOHLCVを返さない場合のYahoo Chart v8同時取得数。既定4、最大6 |
| `TECHNICAL_CACHE_MAX_ENTRIES` | 任意 | 5年履歴キャッシュの最大銘柄数。既定120 |
| `TECHNICAL_FINNHUB_CACHE_TTL_MS` | 任意 | Finnhub決算・ニュース・基本指標のキャッシュ。既定21,600,000ms（6時間） |
| `TECHNICAL_FINNHUB_MAX_SYMBOLS` | 任意 | 1回の分析で補助データを取得する最大銘柄数。既定8 |
